...
import { createEngagement, deleteEngagement, updateEngagement } from '../enagagements';
...
beforeAll(() =>
    Promise.resolve(prisma.project.create({ data: project }))
);

afterAll(() => prisma.project.delete({
    where: {
        id: project.id
    }
}))
...
describe('createEngagement()', () => {
    test('it should create an engagement successfully', async () => {
        ...
        expect(result.startDate.getTime()).toBe(new Date(data.startDate).getTime());
        ...
        result.tasks.sort((firstItem, secondItem) => firstItem.type > secondItem.type ? 1 : 0)
        data.tasks.sort((firstItem, secondItem) => firstItem.type > secondItem.type ? 1 : 0)
        data.tasks.forEach((data_task, index) => {
            const result_task = result.tasks[index];
            expect(result_task.type).toBe(data_task.type)
            expect(result_task.startDate.getTime()).toBe(new Date(data_task.startDate).getTime());
            expect(result_task.dueDate.getTime()).toBe(new Date(data_task.dueDate).getTime());
            expect(result_task.stateHistory[0].stateUpdatedAt.getTime()).toBe(
                new Date(data_task.stateHistory[0].stateUpdatedAt).getTime());
            expect(result_task.stateHistory[0].state).toBe(data_task.stateHistory[0].state)
            expect(result_task.state).toBe("NOT_STARTED")
            expect(result_task.engagementId).toBe(result.id)
        });

        expect(result.attributes.sort((firstItem, secondItem) => firstItem.key > secondItem.key ? 1 : 0)).toEqual(
            data.attributes.sort((firstItem, secondItem) => firstItem.key > secondItem.key ? 1 : 0))
...
describe('updateEngagement()', () => {
    test('it should delete an engagement successfully', async () => {
        ...
        if (result?.completedDate)
            expect(result?.completedDate.getTime()).toBe(new Date(data.completedDate).getTime());
